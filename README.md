...
Скачать и установить все необходимые компоненты в папку /deps/ можно командами
```bat
make download 
make install
```
...
Примечание 1:  
Хотя некоторые пакеты могут изначально присутствовать, для 
...
Для генерации сборочных проектов необходимо выполнить команду 
```bat
make configure
```

В результате данной команнды для плат определенных в переменной PLATFOTRMS из файла environment.mk будут сгенерированы соответсвующие сборочные проекты.


## Сборка и запуск
Сборка проекта осуществляется командой из соответсвующей папки с примером
```mak
 make 
```
